...
export default {
	...
	async mounted() {
		try {
			...
			console.error(e)
			showError(t('test', 'Could not fetch notes'))
		}
		this.loading = false
	},
	...
	methods: {
		...
		async createNote(note) {
			this.updating = true
			try {
				const response = await axios.post(generateUrl('/apps/test/notes'), note)
				const index = this.notes.findIndex((match) => match.id === this.currentNoteId)
				this.$set(this.notes, index, response.data)
				this.currentNoteId = response.data.id
			} catch (e) {
				console.error(e)
				showError(t('test', 'Could not create the note'))
			}
			this.updating = false
		},
		/**
		...
		async updateNote(note) {
			...
			try {
				...
				console.error(e)
				showError(t('test', 'Could not update the note'))
			}
			this.updating = false
		},
		/**
